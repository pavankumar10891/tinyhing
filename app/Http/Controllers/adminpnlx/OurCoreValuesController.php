...
/**
* OurCoreValuesController Controller
*
* Add your methods in the class below
*
*/
class OurCoreValuesController extends BaseController {

	public $model		=	'OurCoreValues';
	public $sectionName	=	'Our Core Values';
	public $sectionNameSingular	= 'Our Core Values';
	...
	/**
	* Function for display all OurCoreValues
	*
	* @param null
	*
	* @return view page. 
    ...
    */
	function save(Request $request){
		...
		if(!empty($formData)){	
			...
			if ($validator->fails()){
				...
				$obj 					=  new OurCoreValues;
				$obj->name 				=  ucfirst($request->input('name'));
				$obj->description 		=  $request->input('description');
				$obj->order_number 		=  $request->input('order_number');
				if($request->hasFile('image')){
                    $extension 			=	 $request->file('image')->getClientOriginalExtension();
                    $fileName			=	time().'-our-core-value-.'.$extension;
                    $folderName     	= 	strtoupper(date('M'). date('Y'))."/";
                    $folderPath			=	OURCOREVALUES_IMAGE_ROOT_PATH.$folderName;
    ...
    public function edit($modelId = 0,Request $request){
		$model		=	OurCoreValues::where('id',$modelId)->first();
		if(empty($model)) {
	...
	*/
	...
	public function update($modelId,Request $request){
		$model					=	OurCoreValues::findorFail($modelId);
		if(empty($model)) {
			return Redirect::back();
		}
		$this->request->replace($this->arrayStripTags($request->all()));
		...
		if(!empty($formData)){
			...
			if ($validator->fails()){
				...
				$obj 		        	=  OurCoreValues::find($modelId);
                ...
                if($request->hasFile('image')){ 
					if(File::exists(OURCOREVALUES_IMAGE_ROOT_PATH.$obj->image)) {
						File::delete(OURCOREVALUES_IMAGE_ROOT_PATH.$obj->image);	
					}
					$extension 		=	$request->file('image')->getClientOriginalExtension();
					$fileName		=	time().'-our-core-value.'.$extension;
					$folderName     = 	strtoupper(date('M'). date('Y'))."/";
	...
	public function view($modelId = 0){
		$model	=	OurCoreValues::where('id',"$modelId")->select('*')->first();      
    ...
    public function delete($userId = 0){
		$userDetails = OurCoreValues::find($userId);
		if(empty($userDetails)) {
			return Redirect::route($this->model.".index");
		}
		if($userId){
			$deleteDate = date("Y-m-d H:i:s");
			OurCoreValues::where('id',$userId)->update(array('is_deleted'=>1, 'deleted_at' => $deleteDate));
