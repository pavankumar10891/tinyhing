...
*
*/
class ExportCustomerMailerReportController extends BaseController {

	public $model				=	'ExportCustomerMailerReportQuatarly';
	public $sectionName			=	'Export Customer Mailer Report Data';
	public $sectionNameSingular	=	'Export Customer Mailer Report';
	...
	public function ExportForm(){
	  ...
	  if(auth()->guard('admin')->user()->user_role == "super_admin"){
		$exportpfs= VendorExportPayment::where('type', 6)->where(['group_id' => Session::get('group')])->get();
	  }else{
		$exportpfs= VendorExportPayment::where('type', 6)->where(['admin_id' => Auth::guard('admin')->user()->id, 'group_id' => Session::get('group')])->get();
	  }
		...
		if(!empty($formData)){
			...
			if ($validator->fails()){
				...
				if(empty($checkExisData)){
					...
					$folderPath = CUSTOMER_QUATERLY_PATH.$folderName;
	...
	{
		$datas =  VendorExportPayment::where('type', 6)->get();
		foreach($datas as $data){
			$file =  storage_path('app/public/pdf/'.$data->export_file);
			if(File::exists($file)){
				unlink($file);
			}
			VendorExportPayment::where('type',6)->where('id', $data->id)->delete();
		}
		return Redirect::route('ExportCustomerMailerReportQuatarly.export');
	}
...
}// end ExportCustomerMailerReportController
