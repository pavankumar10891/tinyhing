										...
										<div class="col-xl-12">	
											<div class="row">
												<div class="col-xl-6">
													...
													<div class="form-group">
														@if($i == 1)
															{!! HTML::decode( Form::label($language->id.'.title',trans("Page Title").'<span class="text-danger"> * </span>')) !!}
															
															{{ Form::text("data[$language->id][title]",isset($multiLanguage[$language->id]['title'])?$multiLanguage[$language->id]['title']:'', ['class' => 'form-control form-control-solid form-control-lg '.($errors->has('title') ? 'is-invalid':'')]) }}
															<div class="invalid-feedback"><?php echo ($i ==  $language_code ) ? $errors->first('title') : ''; ?></div>
															
														@else
															{!! HTML::decode( Form::label($language->id.'.title',trans("Page Title").'<span class="text-danger">  </span>')) !!}
															{{ Form::text("data[$language->id][title]",isset($multiLanguage[$language->id]['title'])?$multiLanguage[$language->id]['title']:'', ['class' => 'form-control form-control-solid form-control-lg ']) }}
														@endif
													</div>
													<!--end::Input-->
												</div>
												<div class="col-xl-12">
													...
													<div class="form-group">
														...
														@if($i == 1)
															{!! HTML::decode( Form::label($language->id.'.body',trans("Description").'<span class="text-danger"> * </span>')) !!}
															
															{{ Form::textarea("data[$language->id][body]",isset($multiLanguage[$language->id]['body'])?$multiLanguage[$language->id]['body']:'', ['class' => 'form-control form-control-solid form-control-lg '.($errors->has('body') ? 'is-invalid':''),'id' => 'body_'.$language->id]) }}
															<div class="invalid-feedback"><?php echo ($i ==  $language_code ) ? $errors->first('body') : ''; ?></div>
															
														@else
															{!! HTML::decode( Form::label($language->id.'.body',trans("Description").'<span class="text-danger">  </span>')) !!}
															{{ Form::textarea("data[$language->id][body]",isset($multiLanguage[$language->id]['body'])?$multiLanguage[$language->id]['body']:'', ['class' => 'form-control form-control-solid form-control-lg','id' => 'body_'.$language->id]) }}
														@endif
													</div>
													<script>
