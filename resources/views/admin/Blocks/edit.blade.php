												...
												<div class="col-xl-6">
													...
													<div class="form-group">
														@if($i == 1)
															...
															{{ Form::text("data[$language->id][name]",$model->name, ['class' => 'form-control form-control-solid form-control-lg '.($errors->has('name') ? 'is-invalid':'')]) }}
															<div class="invalid-feedback"><?php echo ($i ==  $language_code ) ? $errors->first('name') : ''; ?></div>
															
														@else
															{!! HTML::decode( Form::label($language->id.'.name',trans("Name").'<span class="text-danger">  </span>')) !!}
															{{ Form::text("data[$language->id][name]",$model->name, ['class' => 'form-control form-control-solid form-control-lg ']) }}
														@endif
													</div>
													<!--end::Input-->
												</div>
												<div class="col-xl-12">
													...
													<div class="form-group">
														...
														@if($i == 1)
															...
															{{ Form::textarea("data[$language->id][description]",isset($multiLanguage[$language->id]['description'])?$multiLanguage[$language->id]['description']:'', ['class' => 'form-control form-control-solid form-control-lg '.($errors->has('description') ? 'is-invalid':''),'id' => 'description_'.$language->id]) }}
															<div class="invalid-feedback"><?php echo ($i ==  $language_code ) ? $errors->first('description') : ''; ?></div>
															
														@else
															{!! HTML::decode( Form::label($language->id.'.description',trans("Description").'<span class="text-danger">  </span>')) !!}
															{{ Form::textarea("data[$language->id][description]",isset($multiLanguage[$language->id]['description'])?$multiLanguage[$language->id]['description']:'', ['class' => 'form-control form-control-solid form-control-lg','id' => 'description_'.$language->id]) }}
														@endif
													</div>
													<script>
						...
						<div class="row">
							...
							<div class="col-xl-6">
								...
								<div class="form-group">
									...
									@if($model->image != "")	
										<br />
										<a class="fancybox-buttons" data-fancybox-group="button" href="<?php echo $model->image; ?>"><img height="50" width="50" src="<?php echo $model->image; ?>" /></a>
									@endif
								</div>
								<!--end::Input-->
							</div>
