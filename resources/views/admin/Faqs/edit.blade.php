												...
												<div class="col-xl-12">
													...
													<div class="form-group">
														...
														@if($i == 1)
															...
															{{ Form::textarea("data[$language->id][question]",isset($multiLanguage[$language->id]['question'])?$multiLanguage[$language->id]['question']:'', ['class' => 'form-control form-control-solid form-control-lg '.($errors->has('question') ? 'is-invalid':''),'id' => 'question_'.$language->id]) }}
															<div class="invalid-feedback"><?php echo ($i ==  $language_code ) ? $errors->first('question') : ''; ?></div>
															
														@else
															{!! HTML::decode( Form::label($language->id.'.question',trans("question").'<span class="text-danger">  </span>')) !!}
															{{ Form::textarea("data[$language->id][question]",isset($multiLanguage[$language->id]['question'])?$multiLanguage[$language->id]['question']:'', ['class' => 'form-control form-control-solid form-control-lg','id' => 'question_'.$language->id]) }}
														@endif
													</div>
													<script>
												...
												<div class="col-xl-12">
													...
													<div class="form-group">
														...
														@if($i == 1)
															...
															{{ Form::textarea("data[$language->id][answer]",isset($multiLanguage[$language->id]['answer'])?$multiLanguage[$language->id]['answer']:'', ['class' => 'form-control form-control-solid form-control-lg '.($errors->has('answer') ? 'is-invalid':''),'id' => 'answer_'.$language->id]) }}
															<div class="invalid-feedback"><?php echo ($i ==  $language_code ) ? $errors->first('answer') : ''; ?></div>
															
														@else
															{!! HTML::decode( Form::label($language->id.'.answer',trans("answer").'<span class="text-danger">  </span>')) !!}
															{{ Form::textarea("data[$language->id][answer]",isset($multiLanguage[$language->id]['answer'])?$multiLanguage[$language->id]['answer']:'', ['class' => 'form-control form-control-solid form-control-lg','id' => 'answer_'.$language->id]) }}
														@endif
													</div>
													<script>
														/* CKEDITOR for description */
