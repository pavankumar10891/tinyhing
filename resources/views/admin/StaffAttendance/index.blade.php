	...
	<div class="d-flex flex-column-fluid">
		...
		<div class=" container ">
			...
			<div class="row">
							...
							<div>
								<div class="row mb-6">
									...
									<div class="col-lg-4 mb-lg-5 mb-6">
										<label>Name</label>
										{{ Form::text('name',((isset($searchVariable['name'])) ? $searchVariable['name'] : ''), ['class' => ' form-control','placeholder'=>'Name','id'=>'name']) }}
									</div>
									<div class="col-lg-4 mb-lg-5 mb-6">
				...
				<div class="dataTables_wrapper ">
					<div class="table-responsive">
						...
						<thead>
							...
							<th
							class="{{(($sortBy == 'checkin_date' && $order == 'desc') ? 'sorting_desc' : (($sortBy == 'checkin_date' && $order == 'asc') ? 'sorting_asc' : 'sorting'))}}">
							{{
								link_to_route(
								"$modelName.index",
								trans("Checkin Date"),
						...
						<th
						class="{{(($sortBy == 'checkout_time' && $order == 'desc') ? 'sorting_desc' : (($sortBy == 'checkout_time' && $order == 'asc') ? 'sorting_asc' : 'sorting'))}}">
						{{
							link_to_route(
							"$modelName.index",
				...
				<th
					class="{{(($sortBy == 'checkin_time' && $order == 'desc') ? 'sorting_desc' : (($sortBy == 'checkin_time' && $order == 'asc') ? 'sorting_asc' : 'sorting'))}}">
					{{
						link_to_route(
						"$modelName.index",
		...
		<tbody>
			...
			<tr>
				...
				<td>
					<div class="text-dark-75 mb-1 font-size-lg">
						{{ !empty($result->checkin_date)? date('m/d/Y', strtotime($result->checkin_date)):'' }}
					</div>
				</td>
				<td>
					<div class="text-dark-75 mb-1 font-size-lg">
						{{ !empty($result->checkin_time)? $result->checkin_time:'' }}
					</div>
				</td>
				<td>
					<div class="text-dark-75 mb-1 font-size-lg">
						{{ !empty($result->checkout_time)? $result->checkout_time:'' }}
					</div>
				</td>
...
<script>
	...
	$( document ).ready(function() {
		$(document).off('click', '#download_report').on('click', '#download_report', function(){
			var route = "{!! route('payout-request.payoutdownload') !!}?name="+$("#name").val()+"&email="+$("#email").val();
