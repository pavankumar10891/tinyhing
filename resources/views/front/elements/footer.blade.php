...
<section class="counter-sec newletter_wrap">
    <div class="container">
        <div class=" newletter_inner">
            <div class="pr-3">
                <h2> {!! !empty($newsletterHeading->description) ? strip_tags($newsletterHeading->description):'' !!}</h2>
                <h5>{{!empty($newsletterHeading->name) ? $newsletterHeading->name:'' }}</h5>
            </div>
            <div class="newletter-input-wrap">
                <div class="newletter-input">
                   <input name="_token" type="hidden" value="{{ csrf_token() }}"/>
                    {{ Form::text('email','', [ 'id' =>'newletter_email', 'class' => ''.($errors->has('email') ? 'is-invalid':'') , 'placeholder'=>'Enter Your Email Id']) }}  
                    <span id="email_error" class="help-inline error news_email_error"></span>
                </div>
                    
                {{ Form::button('Subscribe Now', ['type' => 'button', 'class' => 'newletter-sub-btn', 'id' => 'newsletter']) }}
...
<footer class="footer_wrapper">
    ...
    <div class="container">
        <div class="copyright">
            <span>
                <a href="{{ url('/') }}"> tinyhugs.net</a> © <?php echo date("Y");?>. All Rights Reserved </span>
        </div>
    </div>
  ...
  <div class="modal-dialog  modal-dialog-centered" role="document">
    <div class="modal-content">
        ...
        <div class="text-center py-4">
            ...
            <div class="btn-block mt-4">
                <a href="{{ url('/our-nannies?type=babbysitter') }}" class="btn-theme proceedNowBtn">Select Babysitter</a>
            </div>
        </div>
        </div>
    </div>
...
<script type="text/javascript">
    $(document).ready(function() {
       ...
       $('#zipcode_search').click(function() {
            ...
            window.location.href="{{url('/our-nannies/?zipcode=')}}"+zipcode;
           // location.reload();
   ...
   //alert(formData);
    ...
    $(".proceedNowBtn").on('click',function(){
        if($("#actionInterview").prop('checked')==true){
            window.location.href="{{route('user.nannylist')}}";
        }else{
